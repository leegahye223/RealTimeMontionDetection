...
int main(int argc, char **argv)
{
	...
	char *ifile_name, *ofile_pt_name, *ofile_st_name, *ofile_turns_name, *train_file_name;
	FILE *fpp, *fps, *fpi, *fpt, *fpturn;
	char *line = NULL;
	size_t len = 0;
	ssize_t read;
	int N_SAMPLES;
	...
	//****************************************START TURN DETECTION************************************************************
		if (j == 5) //run turn detection code only once.
		{
			...
			turn_soft_threshold = overall_turn_mean + calculate_range(turn_mean, n_P_saved[5]-1)/2;
			printf ("turn_soft_threshold = %lf", turn_soft_threshold);
