...
void jump_neural_network()
{
    ...
    int num_low_jump[3] = {0}, num_high_jump[3] = {0}, num_strides[3] = {0};
	int total_low_jumps = 0, total_high_jumps = 0, total_strides = 0;
	...
	// Open the file containing the jump characteristics
	...
	//printf("Jumping training file has %d samples\n", N_SAMPLES);
	...
	for (i = 0; i < N_SAMPLES; i++) {
		...
		if (output1 > output2 && output1 > output3 && output1 > 0.999)
			total_low_jumps++;
		else if (output2 > output1 && output2 > output3 && output2 > 0.999)
			total_high_jumps++;
		else
			total_strides++;
...
void jump_detection(const char *fname)
{
	...
	//char *ifile_name;
	char *ofile_pt_name, *ofile_pt_filtered_name, *ofile_jump_name, *train_file_name;
	FILE *fpp, *fpi, *fpt, *fpj, *fpt_filtered;
	char *line = NULL;
	size_t len = 0;
	ssize_t read;
	int N_SAMPLES;
	...
	double *PT_i_filtered;  // filtered out false jumps
	...
	double air_time_max     = 0.65;
	double jump_range_low   = 2.30;
	...
	ofile_pt_filtered_name = (char *) malloc(sizeof(char) * BUFF_SIZE);
	memset(ofile_pt_filtered_name, 0, BUFF_SIZE);
	
	// File names
	//ifile_name = "walking_data_speed1.csv";
	if (mode == 1)
	{
	...
	else
	{
		...
		ofile_pt_filtered_name = "jump_peaks_troughs_filtered.csv";
	}
	...
	*/
	...
	PT_i_filtered = (double *) malloc(sizeof(double) * N_SAMPLES);
	...
	/* open the output file to write the filtered peaks and troughs */
		//printf("Attempting to write to file \'%s\'.\n", ofile_pt_filtered_name);
		fpt_filtered = fopen(ofile_pt_filtered_name, "w");
		if (fpt_filtered == NULL) {
			fprintf(stderr, 
					"Failed to write to file \'%s\'.\n", 
					ofile_pt_filtered_name
				   );
	...
	//**********************************************START OF JUMP DETECTION ALGORITHM**************************************************************************************************
		...
		if (j == 0)     // Look at Accel X only
		{
			...
			double jump_mag_rise = 0;              // difference between first peak and trough magnitudes
			double jump_mag_fall = 0;              // difference between second peak and trough magnitudes
			...
			// Filter in the trough-to-peak pairs
			k = 0;
			for (i = 0; i < n_PT-1; i++)
			{
				idx = (int) PT_i[i];
				...
				{
					if (x[idx_next] - x[idx] > jump_range_low)
					{
						PT_i_filtered[k] = idx;
						PT_i_filtered[k+1] = idx_next;
						k += 2;
			...
			for (i = 0; i < n_PT-3; i++)
			{	
				idx  = (int) PT_i_filtered[i];
				idx2 = (int) PT_i_filtered[i+1];
				idx3 = (int) PT_i_filtered[i+2];
				idx4 = (int) PT_i_filtered[i+3];
				
				// 2 peaks and 2 troughs constitute a jump
				// First trough followed by first peak
				if ((x[idx] < center) && (x[idx2] > center))     
				{
					// Must satisfy timing and threshold requirements
					jump_mag_rise = x[idx2] - x[idx];
					if (((t[idx2] - t[idx]) < jump_time) && (jump_mag_rise > jump_range_low) && (jump_mag_rise < jump_range_high))
					{
						// Second trough followed by second peak
						if ((x[idx3] < center) && (x[idx4] > center) && ((t[idx3] - t[idx2]) < air_time_max))     
						{
							// Must satisfy timing and threshold requirements
							jump_mag_fall = x[idx4] - x[idx3];
							if (((t[idx4] - t[idx3]) < jump_time) && (jump_mag_fall > jump_range_low) && (jump_mag_fall < jump_range_high))
							{
								J_i[n_J] = idx;
								J_i[n_J+1] = idx2;
								J_i[n_J+2] = idx3;
								J_i[n_J+3] = idx4;
								n_J += 4;
								i += 3;  // Since 4 peaks/troughs are added, skip to the next peak/trough to examine
							}
						}
					}
				}
		...
		// Label the data sets for filtered peaks and troughs
		switch(j)
		{
			case 0: fprintf(fpt_filtered, "Accel X\n");
					break;
			case 1: fprintf(fpt_filtered, "Accel Y\n");
					break;
			case 2: fprintf(fpt_filtered, "Accel Z\n");
					break;
			case 3: fprintf(fpt_filtered, "Gyro X\n");
					break;
			case 4: fprintf(fpt_filtered, "Gyro Y\n");
					break;
			case 5: fprintf(fpt_filtered, "Gyro Z\n");
					break;
		}
		
		// Write filtered peaks and troughs if available
		fprintf(fpt_filtered, "PT_i,PT_t,PT_x\n");
		for (i = 0; i < n_PT; i++) {
			idx = (int) PT_i[i];
			fprintf(fpt_filtered, "%d,%20.10lf,%lf\n",
					idx,
					t[idx],
					x[idx]
				   );
		}
		fprintf(fpt_filtered, "\n\n\n");
		//fclose(fpt_filtered);
...
int main()
{
	...
	{
		fname = "walking4jump.csv";
		jump_detection(fname);
		
		//fname = "data_2.csv";
		//jump_detection(fname);
		
		//fname = "walk_speed1.csv";
		//jump_detection(fname);
		
		//fname = "walk_speed2.csv";
		//jump_detection(fname);
		
		//fname = "walk_speed3.csv";
		//jump_detection(fname);
		
		//fname = "walk_speed4.csv";
		//jump_detection(fname); 
	}
